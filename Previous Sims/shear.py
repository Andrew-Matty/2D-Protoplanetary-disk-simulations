...
# Problem
#Gas
problem = d3.IVP([u, v, s, p, tau_p], namespace=locals())
problem.add_equation("dt(u) + grad(p) - nu*lap(u) = - u@grad(u) + s*(v-u)/0.01")
problem.add_equation("dt(v) - nu*lap(v) = - v@grad(v) - (v-u)/0.01")
problem.add_equation("dt(s) - D*lap(s) = - v@grad(s) - s*div(v)")
...
v['g'][1] += 0.1 * np.sin(2*np.pi*x/Lx) * np.exp(-(z-0.5)**2/0.01)
