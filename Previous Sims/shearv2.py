...
#Gravity Field
gravity = dist.VectorField(coords,name='forcing',bases=(xbasis,zbasis))
gravity['g'] = -(z**2)*(z/abs(z))
...
problem.add_equation("dt(u) - nu*div(grad_u) + grad(p) + lift(tau_u2) = - u@grad(u) + s*(v-u)/0.01 + gravity")
problem.add_equation("dt(v) - nu*div(grad_v)           + lift(tau_v2) = - v@grad(v) -   (v-u)/0.01 + gravity")

#Gas BC
problem.add_equation("u(z=-Lz/2) = 0.5*ex")
problem.add_equation("u(z=+Lz/2) = 0.5*ex")
...
problem.add_equation("dz(s)(z=+Lz/2) = 0.0") # Dust-to-gas ratio
problem.add_equation("v(z=-Lz/2) = -0.5*ex")
problem.add_equation("v(z=+Lz/2) = -0.5*ex")
...
#s['g'] +=  2 * (-(np.tanh((z-0.1)/0.5)) + np.tanh((z+0.1)/0.5))#0.1-previously
s['g'] += -(1/3 * (np.tanh((z-0.5)/0.1) - np.tanh((z+0.5)/0.1)))
