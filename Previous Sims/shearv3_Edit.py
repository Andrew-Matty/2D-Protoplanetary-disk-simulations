...
problem.add_equation("dt(u) - nu*div(grad_u) + grad(p) + lift(tau_u2) = 2*(dot(u,ez)+eta)*Lambda*ex-(2-q)*Lambda*(dot(u,ex))*ez - u@grad(u) + s*(v-u)/ts + gravity*ez - p*dz(log_rho)*ez")
problem.add_equation("dt(v) - nu*div(grad_v)           + lift(tau_v2) = 2*(dot(v,ez))*Lambda*ex - (2-q)*Lambda*(dot(u,ex))*ez - v@grad(v) -   (v-u)/ts + gravity*ez")
...
delta = 1
kx, kz = 1, 1
sinpert = 1 + delta*np.cos(2*np.pi*kx*x/Lx)*np.cos(2*np.pi*kz*z/Lz)

# Background shear
#u['g'][0] = 1/2 + 1/2 * (np.tanh((z-0.5)/0.1) - np.tanh((z+0.5)/0.1))
#v['g'][0] = 1/2 + 1/2 * (np.tanh((z-0.5)/0.1) - np.tanh((z+0.5)/0.1))
u['g'][0] = -sinpert*(2*ts*eta)/((1+eps)**2+ts**2)
v['g'][0] = sinpert*(2*eps*ts*eta)/((1+eps)**2+ts**2)
...
s['g'] += initialrho*(1-0.99*np.tanh(abs(z/0.5)))
