...
problem.add_equation("dt(u) - nu*div(grad_u) + grad(p) + lift(tau_u2) = 2*(dot(u,ey)+eta)*Lambda*ex - (2-q)*Lambda*dot(u,ex)*ey - u@grad(u) + s*(v-u)/ts + gravity*ez - p*dz(log_rho)*ez")
problem.add_equation("dt(v) - nu*div(grad_v) + lift(tau_v2) = 2*dot(v,ey)*Lambda*ex - (2 - q)*Lambda*dot(u,ex)*ey- v@grad(v) -  (v-u)/ts + gravity*ez")
...
#init conditions trial
delta = 1
kx, kz = 1, 1
func = 1 + delta*np.cos((2*np.pi*kx*x)/Lx)*np.cos((2*np.pi*kz*z)/Lz)
u['g'][0] = func*((2*eps*ts*eta)/(1+eps)**2+ts**2)
u['g'][1] = func*eta*(((eps*(1+eps))/(1+eps)**2+ts**2)-1)
v['g'][0] = -func*((2*ts*eta)/(1+eps)**2+ts**2)
v['g'][1] = -func*((eta*(1+eps))/(1+eps)**2+ts**2)
# modding the [1] as y as they are 0 atm so shall see if this works with eq

# Dust
s.fill_random('g', seed=42, distribution='normal', scale=0.01)
s.low_pass_filter(scales=0.5)
init=0.1
s ['g'] += init*(1-0.99*np.tanh(abs(z/0.5)))
...
snapshots.add_task(s(y=0), name='tracer')
snapshots.add_task(p(y=0), name='pressure')
snapshots.add_task(p(y=0), name='vorticity')
